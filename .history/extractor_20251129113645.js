...
const lockFile = path.resolve(__dirname, '.extractor.lock');
...
try {
  if (fs.existsSync(lockFile)) {
    console.log('Another extractor run is in progress, exiting.');
    process.exit(0);
  }
  fs.writeFileSync(lockFile, String(process.pid));
} catch (err) {
  console.error('Failed to create lock file, exiting.', err);
  process.exit(1);
}
